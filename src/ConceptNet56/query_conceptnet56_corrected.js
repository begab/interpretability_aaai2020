...
const fname_corrected_dl     = './corrected/glove300d_l_0.5_DL_top400000.emb.gz/top_bot_words.jsons';
const fname_corrected_gs     = './corrected/glove300d_l_0.5_GS_top400000.emb.gz/top_bot_words.jsons';
const fname_corrected_kmeans = './corrected/glove300d_l_0.5_kmeans_top400000.emb.gz/top_bot_words.jsons';
...
MongoClient.connect("mongodb://localhost:27017/", { useNewUrlParser: true }, function(init_err, db) {
	...
	*/
	...
	/**/
	load_top_bottom_words_and_perform_spreading_activation_tests_no_s(dbo, fname_top_bottom_glove_kmeans, fname_corrected_kmeans, function (err) {
		if (err) throw err;
...
function load_top_bottom_words_and_perform_spreading_activation_tests_no_s (db_object, fname, fname_corrections, callback) {
	...
	fs.readFile(fname, {encoding: 'utf8', flag: 'r'}, function (err_read, data_read) {
		...
		//console.log(JSON.stringify(base_info[6]));
		console.log(" * PATCHING THE INPUT FILE; loading corrections for some bases");
		load_corrections (base_info, fname_corrections, function (some_err, base_indices) {
			console.log(" * PATCHING THE INPUT FILE; DONE using file " + fname_corrections + "\n");
			//console.log(JSON.stringify(base_info[6]));
			
			/*
			 * Split the base_indices into four/eight different arrays;
			 * This way I can start manually four processes in parallel
			 * and observe the outcome in each terminal.
			 */
			//var difficult_base_indices_dl_corrected = [32, 396, 587, 867, 925];
			//var difficult_base_indices_gs_corrected = [72, 884];
			var difficult_base_indices_kmeans_corrected = [186 /* less than 10 */, 751, 781];
			var array_size = Math.floor(base_indices.length / 8.0);
			var array_1 = [], array_2 = [], array_3 = [], array_4 = [], array_5 = [], array_6 = [], array_7 = [], array_8 = [];
			for (i = 0; i < array_size; i++) {
				array_1.push(base_indices[i]);
				array_2.push(base_indices[array_size + i]);
				array_3.push(base_indices[2*array_size + i]);
				/**/
				array_4.push(base_indices[3*array_size + i]);
				array_5.push(base_indices[4*array_size + i]);
				array_6.push(base_indices[5*array_size + i]);
				array_7.push(base_indices[6*array_size + i]);
				/**/
			}
			for (i = 0; i < (base_indices.length - 7*array_size); i++) {
				array_8.push(base_indices[7 * array_size + i]);
			}
			assert.ok(7*array_size + array_8.length === base_indices.length);

			var base_to_avoid, temp_index;
			for (i = 0; i < difficult_base_indices_kmeans_corrected.length; i++) {
				
				base_to_avoid  = difficult_base_indices_kmeans_corrected[i];

				temp_index     = array_1.indexOf(base_to_avoid);
				if (temp_index !== -1) array_1.splice(temp_index, 1);
				temp_index     = array_2.indexOf(base_to_avoid);
				if (temp_index !== -1) array_2.splice(temp_index, 1);
				temp_index     = array_3.indexOf(base_to_avoid);
				if (temp_index !== -1) array_3.splice(temp_index, 1);
				temp_index     = array_4.indexOf(base_to_avoid);
				if (temp_index !== -1) array_4.splice(temp_index, 1);
				temp_index     = array_5.indexOf(base_to_avoid);
				if (temp_index !== -1) array_5.splice(temp_index, 1);
				temp_index     = array_6.indexOf(base_to_avoid);
				if (temp_index !== -1) array_6.splice(temp_index, 1);
				temp_index     = array_7.indexOf(base_to_avoid);
				if (temp_index !== -1) array_7.splice(temp_index, 1);
				temp_index     = array_8.indexOf(base_to_avoid);
				if (temp_index !== -1) array_8.splice(temp_index, 1);
			}
			//assert.ok(difficult_base_indices_dl_corrected.length + array_1.length + array_2.length + array_3.length + array_4.length === base_indices.length);
			//assert.ok(difficult_base_indices_gs_corrected.length + array_1.length + array_2.length + array_3.length + array_4.length + array_5.length + array_6.length + array_7.length + array_8.length === base_indices.length);
			assert.ok(difficult_base_indices_kmeans_corrected.length + array_1.length + array_2.length + array_3.length + array_4.length + array_5.length + array_6.length + array_7.length + array_8.length === base_indices.length);


			var start_time = process.hrtime();
			compute_heavy_paths_for_particular_bases(db_object, base_info, array_7, function (err_comp, res_comp) {
				console.log("\n\n ======================================== DONE ========================================\n");
...
function load_corrections (base_info, fname_corrections, callback) {
	var i, current_object, current_base, bases_of_interest = [];
	fs.readFile(fname_corrections, {encoding: 'utf8', flag: 'r'}, function (err_read, data_read) {
		if (err_read) throw err_read;

		var some_bases_as_strings = data_read.split('\n');
		for (i = 0; i < some_bases_as_strings.length; i++) {
			current_object = JSON.parse(some_bases_as_strings[i]);
			current_base   = current_object.base_id;
			base_info[current_base] = current_object;
			bases_of_interest.push(current_base);
		}
		callback(null, bases_of_interest);
	});
}
