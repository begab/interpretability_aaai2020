			...
			compute_stats_recursively (results, 0, "", function (err3, file_contents) {
				...
				write_stats_to_file(file_contents, function (err4) {
					...
					for (i = 0; i < data.length; i++) {
						...
						top_max_array.push(parseInt(data[i].top.max));
						bot_max_array.push(parseInt(data[i].bot.max));
						...
						if (parseInt(data[i].top.max) < parseInt(data[i].bot.max)) {
							top_max_less += 1;
						} else if (parseInt(data[i].top.max) > parseInt(data[i].bot.max)) {
							bot_max_less += 1;
						} else { 
							console.log("base: " + data[i].base + ", top: " + data[i].top.max + " max: " + data[i].bot.max);
							max_ties += 1;
						}
					}
